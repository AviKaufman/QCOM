...
def _needs_complex(phi: np.ndarray) -> bool:
    ...
    return bool(np.any(np.sin(phi) != 0.0))
...
class RydbergHamiltonian(BaseHamiltonian):
    ...
    def dtype(self) -> np.dtype:
        ...
        return np.dtype(np.complex128 if _needs_complex(self.params.phi) else np.float64)
    ...
    def _diagonal(self) -> np.ndarray:
        if self._diag_cache is None:
            dtype = self.dtype
            diag = _detuning_diagonal_from_bits(self.num_sites, self.params.delta, dtype)
            if np.any(self.params.vij):
                diag += _interaction_diagonal_from_bits(self.num_sites, self.params.vij, dtype)
            self._diag_cache = np.ascontiguousarray(diag, dtype=dtype)
        return self._diag_cache

    def _matvec(self, psi: np.ndarray) -> np.ndarray:
        """
        Compute H @ psi without materializing H.
        """
        dtype = self.dtype
        psi = np.asarray(psi, dtype=dtype, order="C")
        ...
        for i in range(self.num_sites):
            ...
            up_val = (self.params.omega[i] * 0.5) * (ci - 1j * si)
            low_val = (self.params.omega[i] * 0.5) * (ci + 1j * si)
            if dtype == np.dtype(np.float64):
                up_val = float(np.real(up_val))
                low_val = float(np.real(low_val))
            out[bit_is_zero] += up_val * psi[flipped[bit_is_zero]]
    ...
    def to_dense(self) -> np.ndarray:
        ...
        for i in range(N):
            ...
            n_i = 0.5 * (
                _kron_local_dense(N, i, _I2, dtype) - _kron_local_dense(N, i, _SIGMA_Z, dtype)
            )
            n_ops.append(n_i)
...
def _drive_coo_from_bitflips(
    ...
    for i in range(N):
        ...
        rows.append(m)
        cols.append(n)
        data.append(np.full(m.size, up_val, dtype=np.complex128))
        rows.append(n)
        cols.append(m)
        data.append(np.full(m.size, low_val, dtype=np.complex128))
    ...
    if not any_complex:
        data_arr = data_arr.real.astype(np.float64, copy=False)
        return rows_arr, cols_arr, data_arr, np.dtype(np.float64)

    return rows_arr, cols_arr, data_arr, np.dtype(np.complex128)
