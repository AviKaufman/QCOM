...
def eigensolve(
    ...
    *,
    ...
    with (
        ProgressManager.progress("Eigen solve", total_steps=1)
        if show_progress
    ...
    if return_result:
        return SpectrumResult(
            eigenvalues=evals,
            eigenvectors=evecs if return_eigenvectors else None,
            metadata={"which": which, "sigma": sigma, "hermitian": use_herm},
        )
...
def ground_state(
    ...
    *,
    ...
    if return_result:
        return SpectrumResult(
            eigenvalues=evals[:1],
            eigenvectors=evecs[:, :1] if return_vector else None,
            metadata={"which": which, "kind": "ground_state"},
        )
    if return_vector:
        ...
        if return_eigenvectors:
            ...
            if return_result:
                return SpectrumResult(
                    eigenvalues=evals,
                    eigenvectors=evecs,
                    metadata={"method": "dense", "hermitian": True},
                )
            return evals, evecs
    ...
    if return_eigenvectors:
        ...
        if return_result:
            return SpectrumResult(
                eigenvalues=evals,
                eigenvectors=evecs,
                metadata={"method": "dense", "hermitian": False},
            )
        return evals, evecs
