...
def order_dict(inp_dict: Dict[str, float]) -> Dict[str, float]:
    ...
    for k in inp_dict.keys():
        if not isinstance(k, str) or len(k) == 0 or any(c not in "01" for c in k):
            raise ValueError(
                f"order_dict: invalid binary key {k!r} (expected non-empty '0'/'1' string)."
            )
...
def part_dict(inp_dict: Dict[str, float], indices: Iterable[int]) -> Dict[str, float]:
    ...
    for k in inp_dict.keys():
        if not isinstance(k, str) or len(k) == 0 or any(c not in "01" for c in k):
            raise ValueError(
                f"part_dict: invalid binary key {k!r} (expected non-empty '0'/'1' string)."
            )
        lengths.add(len(k))
    ...
    if len(lengths) != 1:
        raise ValueError(
            f"part_dict: all keys must have equal length; got lengths {sorted(lengths)}."
        )
    L = lengths.pop()
    ...
    if any(i < 0 or i >= L for i in normalized_indices):
        raise ValueError(
            f"part_dict: indices out of bounds after normalization for key length {L}: {indices_tuple} → {normalized_indices}."
        )
