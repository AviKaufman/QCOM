...
def test_mutual_information_zero_for_independent_bits():
    ...
    mutual_info = compute_mutual_information(probs, configuration=[0, 1], base=2)
    result = compute_mutual_information(probs, configuration=[0, 1], base=2, return_components=True)
    # Independence ⇒ I(A:B)=0 and H(AB)=H(A)+H(B)
    assert _isclose(mutual_info, 0.0, atol=1e-12)
    assert isinstance(result, MutualInformationResult)
    assert _isclose(result.h_ab, result.h_a + result.h_b)
...
def test_mutual_information_positive_for_perfect_correlation():
    ...
    assert _isclose(result.h_a, 1.0)
    assert _isclose(result.h_b, 1.0)
    assert _isclose(result.h_ab, 1.0)
    assert _isclose(mutual_info, 1.0)
    assert _isclose(result.mutual_information, 1.0)


def test_mutual_information_indices_argument_path():
    # Use explicit indices instead of configuration
    probs = {"00": 0.5, "11": 0.5}
    mutual_info = compute_mutual_information(probs, a_indices=[0], b_indices=[1], base=2)
    assert _isclose(mutual_info, 1.0)
